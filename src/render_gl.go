...
func (r *Renderer_GL21) newTexture(width, height, depth int32, filter bool) Texture {
	...
	gl.GenTextures(1, &h)
	t := &Texture_GL21{width, height, depth, [2]int32{0, 0}, [4]float32{0, 0, 1, 1}, filter, h}
	format := t.MapInternalFormat(Max(depth, 8))
	gfx.(*Renderer_GL21).BindCurrent2DTexture(h)
...
func (r *Renderer_GL21) newPaletteTexture(slot [2]uint32, offset [2]int32, uvst [4]float32) Texture {
	t := &Texture_GL21{256, 1, 32, offset, uvst, false, 0}
	t.handle = (*(sys.palTexture.textures[slot[0]])).(*Texture_GL21).handle
	runtime.SetFinalizer(t, func(t *Texture_GL21) {
		sys.mainThreadTask <- func() {
...
func (r *Renderer_GL21) newSubTexture(tex Texture, width, height int32, uvst [4]float32) Texture {
	t := &Texture_GL21{width, height, tex.(*Texture_GL21).depth, [2]int32{0, 0}, uvst, tex.(*Texture_GL21).filter, tex.(*Texture_GL21).handle}
	return t
}
...
func (r *Renderer_GL21) newDataTexture(width, height int32) (t Texture) {
	...
	gl.ActiveTexture(gl.TEXTURE0)
	gl.GenTextures(1, &h)
	t = &Texture_GL21{width, height, 128, [2]int32{0, 0}, [4]float32{0, 0, 1, 1}, false, h}
	runtime.SetFinalizer(t, func(t *Texture_GL21) {
		sys.mainThreadTask <- func() {
			gl.DeleteTextures(1, &t.handle)
		}
...
func (r *Renderer_GL21) newHDRTexture(width, height int32) (t Texture) {
	...
	gl.ActiveTexture(gl.TEXTURE0)
	gl.GenTextures(1, &h)
	t = &Texture_GL21{width, height, 96, [2]int32{0, 0}, [4]float32{0, 0, 1, 1}, false, h}
	runtime.SetFinalizer(t, func(t *Texture_GL21) {
		sys.mainThreadTask <- func() {
			gl.DeleteTextures(1, &t.handle)
		}
...
func (r *Renderer_GL21) newCubeMapTexture(widthHeight int32, mipmap bool, lowestMipLevel int32) (t Texture) {
	...
	gl.ActiveTexture(gl.TEXTURE0)
	gl.GenTextures(1, &h)
	t = &Texture_GL21{widthHeight, widthHeight, 24, [2]int32{0, 0}, [4]float32{0, 0, 1, 1}, false, h}
	runtime.SetFinalizer(t, func(t *Texture_GL21) {
		sys.mainThreadTask <- func() {
			gl.DeleteTextures(1, &t.handle)
		}
	})
	gfx.(*Renderer_GL21).BindCurrentCubeTexture(h)
	for i := 0; i < 6; i++ {
...
func (t *Texture_GL21) SetSubData(data []byte, x, y, width, height int32) {
	...
	gfx.(*Renderer_GL21).BindCurrent2DTexture(t.handle)
...
func (t *Texture_GL21) GetUV() [4]float32 {
	return t.uvst
}
...
func (r *Renderer_GL21) prepareModelPipeline(bufferIndex uint32, env *Environment) {
	...
	if r.enableShadow {
		for i := 0; i < 4; i++ {
			...
			switched := r.BindCubeTexture(unit, r.fbo_shadow_cube_texture[i])
			if switched {
	...
	if env != nil {
		...
		switched := r.BindCubeTexture(unit, env.lambertianTexture.tex.(*Texture_GL21).handle)
		if switched {
			gl.Uniform1i(loc, int32(unit))
		}
		loc, unit = r.modelShader.u["GGXEnvSampler"], r.modelShader.t["GGXEnvSampler"]
		switched = r.BindCubeTexture(unit, env.GGXTexture.tex.(*Texture_GL21).handle)
		if switched {
			gl.Uniform1i(loc, int32(unit))
		}
		loc, unit = r.modelShader.u["GGXLUT"], r.modelShader.t["GGXLUT"]
		switched = r.Bind2DTexture(unit, env.GGXLUT.tex.(*Texture_GL21).handle)
		if switched {
			gl.Uniform1i(loc, int32(unit))
		}
...
func (r *Renderer_GL21) Bind2DTexture(index int, tex uint32) bool {
...
func (r *Renderer_GL21) BindCubeTexture(index int, tex uint32) bool {
	if r.textures[index] != tex || tex == 0 {
		r.textures[index] = tex
		if r.textureIndex != index {
			r.textureIndex = index
...
func (r *Renderer_GL21) BindCurrent2DTexture(tex uint32) bool {
...
func (r *Renderer_GL21) BindCurrentCubeTexture(tex uint32) bool {
	if r.textures[r.textureIndex] != tex {
		r.textures[r.textureIndex] = tex
		gl.BindTexture(gl.TEXTURE_CUBE_MAP, tex)
		return true
