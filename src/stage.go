	...
	if doc.Extensions != nil {
		if lightExtension, ok := doc.Extensions["KHR_lights_punctual"]; ok {
			...
			for _, light := range ext.(map[string]interface{})["lights"].([]interface{}) {
				...
				lightType := light.(map[string]interface{})["type"].(string)
				switch lightType {
				case "point":
				...
				if intensity, ok := light.(map[string]interface{})["intensity"]; ok {
					newLight.intensity = (float32)(intensity.(float64))
				}
				if lightRange, ok := light.(map[string]interface{})["range"]; ok {
					newLight.lightRange = (float32)(lightRange.(float64))
				}
				if outerConeAngle, ok := light.(map[string]interface{})["outerConeAngle"]; ok {
					newLight.outerConeAngle = (float32)(outerConeAngle.(float64))
				}
				if innerConeAngle, ok := light.(map[string]interface{})["innerConeAngle"]; ok {
				...
				if color, ok := light.(map[string]interface{})["color"]; ok {
...
func (s *Stage) drawModel(pos [2]float32, yofs float32, scl float32, sceneNumber int) {
	...
	if len(scene.lightNodes) > 0 && sceneNumber == 0 {
		...
		for i := 0; i < int(Min(int32(len(scene.lightNodes)), 8)); i++ {
			light := s.model.nodes[scene.lightNodes[i]]
			lightProj := mgl.Perspective(mgl.DegToRad(90), 1, 0.1, 50)
			if s.model.lights[*light.lightIndex].lightType == DirectionalLight {
				lightProj = mgl.Ortho(-20, 20, -20, 20, -20, 50)
			...
			if s.model.lights[*light.lightIndex].lightType == PointLight {
				...
				gfx.SetShadowMapUniformI("isPointLight", 1)
				gfx.SetShadowMapUniformF("farPlane", 50)
				...
				gfx.SetShadowMapUniformI("isPointLight", 0)
				gfx.SetShadowMapUniformF("farPlane", 50)
			}
